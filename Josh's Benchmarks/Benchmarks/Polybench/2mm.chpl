...
proc kernel_2mm(alpha: int, beta: int, distribution, matrix_size: int) {
	...
	//Matrices to check correctness of calculation
	...
	//print out communication (gets and puts)
	if messages {
		var messages=0;
	...
	if correct {
	    forall (i,j) in distribution {
	        ...
	        Ctest[i,j] = alpha * (+ reduce (tempArray));
	    }
    
	    forall (i,j) in distribution {
	        var tempArray: [1..matrix_size] real;
	        forall (d, c, k) in zip(Dtest[i, 1..matrix_size], Ctest[1..matrix_size, j], 1..) {
				var temp = d * c;
				tempArray[k] = temp;
	        }
