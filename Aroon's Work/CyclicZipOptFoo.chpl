...
iter CyclicZipOptArr.these(param tag: iterKind, followThis, param fast: bool = false) var where tag == iterKind.follower {	
	...
	if arrSection.locale.id == here.id { //if mem is local
		...
		//helper function that Aroon added to find minimum of two numbers
		...
		//Aroon added code starting here
		//perform strip mining on bufsize to reduce memory footpirnt
		//only works in 1 dimension
		config param bound = 131072;	//bound size of maximum number of elements allowed in buffer. 2^17 buffer elements ~ 1MB		
		for i in 0..bufsize by chunk {
			count(rank+1) = min(chunk, bufsize - i):int(32);
			var buf: [1..min(chunk, bufsize - i)] this.eltType;
