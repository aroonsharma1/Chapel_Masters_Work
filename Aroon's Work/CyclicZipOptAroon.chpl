...
//bound size of maximum number of elements 
//allowed in buffer. 2^17 buffer elements ~ 1MB	
...
proc CyclicZipOpt.dsiCreateRankChangeDist(param newRank: int, args) {
	...
	var collapsedLocs: _CyclicZipOpt_matchArgsShape(range(int), int, args);
...
proc CyclicZipOptArr.dsiRankChange(d, param newRank: int, param stridable: bool, args) {
	...
	coforall ind in d.dist.targetLocDom {
		on d.dist.targetLocs(ind) {
			...
			var locSlice: _CyclicZipOpt_matchArgsShape(range(idxType=idxType, stridable=true), idxType, args);
...
proc CyclicZipOptArr.setupRADOpt() {
	if !stridable { // for now, no support for strided CyclicZipOpt arrays
...
// 	var sum = origin;
// 	if stridable {
// 		halt("RADOpt not supported for strided CyclicZipOpt arrays.");
// 	} else {
// 		for param i in 1..rank do {
...
iter CyclicZipOptArr.these(param tag: iterKind, followThis, param fast: bool = false) var where tag == iterKind.follower {	
	...
	// myFollowThis is a domain consisting of the multdimensional
	// rectangular (possibly strided) region of the array we
	// are to yield in the original coordinate system.
	...
	// arrSection is a LocCyclicArr class instance
	...
	if arrSection.locale.id == here.id { //if mem is local
		...
		//convert count array to a tuple so memory bounding
		...
		for i in go_through_chunks(rank, chunks_tuple, count_tuple, bounded_count_tuple) {
			...
			//size of buffer is now given by i.bounded_chunk_count
			...
			__primitive("chpl_comm_get_strd",
				...
				rid,
				//arrSection is the array (LocCyclicArr) 
				//containing the elements
				//local to a particular locale
				//arrSection.myElems is the Chapel array
				//arrSection.myElems._value goes from Chapel array
			...
			//writeln("Number of elements in buffer = " + num_elements(rank, i.bounded_chunk_count));
		}
...
//(default rectangular array = CyclicZipOpt distributed array)
...
//(CyclicZipOpt distributed array = default rectangular)
