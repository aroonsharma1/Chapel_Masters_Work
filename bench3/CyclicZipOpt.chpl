...
iter CyclicZipOptArr.these(param tag: iterKind, followThis, param fast: bool = false) var where tag == iterKind.follower {
	...
	if arrSection.locale.id == here.id { //if mem is local
		...
		for i in 1..rank {
			...
			srcStride(i)=v.blk(rank-i+1):int(32);
			count(i+1)=nslice(rank-i+1):int(32);
		}
		
		var buf: [1..bufsize] this.eltType;
		var dest = buf._value.data;
		const src = arrSection.myElems._value.data;
		...
		if testzipopt {
			var j=1;
			for i in myFollowThis {
				if buf[j] != accessHelper(i) {
					writeln('comm get wrong ', here.id, ' t=', t, ' have=', buf[j], ' expected=', accessHelper(i));
				}
				j+=1;
			}
		}
		...
		if testzipopt {
			...
			for i in myFollowThis {
				if buf[j] != accessHelper(i) {
					writeln('comm put wrong ', here.id, ' t=', t, ' have=', buf[j], ' expected=', accessHelper(i));
				}
				j+=1;
			}
		}
