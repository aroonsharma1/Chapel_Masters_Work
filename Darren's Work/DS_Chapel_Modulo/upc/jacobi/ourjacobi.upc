...
/*			for(iter=0, j=MYTHREAD?MYTHREAD:THREADS; j<ngrid-2; iter++, j+=THREADS) {
				buf1[iter]=u[i][j-1];
			}*/
...
/*			for(iter=0, j=MYTHREAD?MYTHREAD:THREADS; j<ngrid-2; iter++, j+=THREADS) {
			...
			//upc_memget(buf2, &u[i][MYTHREAD?MYTHREAD+1:THREADS+1], sizeof(double)*ngrid);
			...
			//all these are known to be local (cast to local for speedup? todo)
			//original bounds = 1 to ngrid-1
			for(iter=0, j=MYTHREAD?MYTHREAD:THREADS; j<ngrid-1; iter++, j+=THREADS) {
//				double utmp = 0.25*(u[i+1][j]+u[i-1][j]+buf2[iter]+buf1[iter]-h*h*f[i][j]);
